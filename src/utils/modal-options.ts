...
type MainModalOptionsDeps = {
	...
	confirmMarkAsTodo: (projectId: string) => Promise<void>;
};

type TrashModalOptionsDeps = {
	restoreFromTrash: (projectId: string) => Promise<void>;
	deletePermanently: (projectId: string) => Promise<void>;
};

export const useMainProjectModalOptions = ({ section, openEditProjectAlert, confirmMoveToTrash, confirmMarkAsTodo }: MainModalOptionsDeps) => {
	...
	const restoreActiveProject = async () => {
		...
		closeProjectMenu();
		await confirmMarkAsTodo(project.id);
	};
...
export const useTrashProjectModalOptions = ({ restoreFromTrash, deletePermanently }: TrashModalOptionsDeps) => {
	const projectMenuOpen = ref(false);
	const projectMenuEvent = ref<Event | undefined>(undefined);
	const activeProject = ref<Project | null>(null);
